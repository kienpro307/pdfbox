...
public class PDDocument implements Closeable
{
    ...
    private final Collection<COSObjectKey> indirectObjectKeys = new HashSet<COSObjectKey>();
                                         ...
                                         COSDocument visualSignature)
    {
        ...
        // get all objects
        List<COSObject> cosObjects = new ArrayList<COSObject>();
        for (COSObjectKey key : visualSignature.getXrefTable().keySet()) {
            cosObjects.add(visualSignature.getObjectFromPool(key));
        }
    ...
    private void setHighestImportedObjectNumber(PDPage importedPage)
    {
        ...
        long highestImportedNumber = 0L;
        for (COSObjectKey key : indirectObjectKeys) {
            long number = key.getNumber();
            if (number > highestImportedNumber) {
                highestImportedNumber = number;
            }
        }
