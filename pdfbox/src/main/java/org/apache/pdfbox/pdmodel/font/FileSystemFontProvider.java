...
//    }
    ...
    private void addTrueTypeCollection(final File ttcFile) throws IOException
    {
        ...
        try
        {
            final String hash = SKIP_CHECKSUMS ? CHECKSUM_PLACEHOLDER : computeHash(Files.newInputStream(ttcFile.toPath()));
            TrueTypeCollection.processAllFontHeaders(ttcFile, new TrueTypeCollection.TrueTypeFontHeadersProcessor() {
                @Override
                public void process(FontHeaders fontHeaders) {
                    addTrueTypeFontImpl(fontHeaders, ttcFile, hash);
                }
            });
        }
        catch (IOException e)
