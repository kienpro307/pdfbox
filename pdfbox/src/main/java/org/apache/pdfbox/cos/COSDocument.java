...
public class COSDocument extends COSBase implements Closeable
{
    ...
    public COSDictionary getLinearizedDictionary()
    {
        ...
        // within the pdf
        List<COSObjectKey> objectKeys = new ArrayList<COSObjectKey>();
        for (Map.Entry<COSObjectKey, Long> entry : xrefTable.entrySet()) {
            if (entry.getValue() > 0L) {
                objectKeys.add(entry.getKey());
            }
        }
        Collections.sort(objectKeys, new Comparator<COSObjectKey>() {
            public int compare(COSObjectKey o1, COSObjectKey o2) {
                return xrefTable.get(o1).compareTo(xrefTable.get(o2));
            }
        });
    ...
    /**
     * Indicates if an encrypted pdf is already decrypted after parsing.
     *
    ...
    public COSObject getObjectFromPool(COSObjectKey key)
    {
        ...
        if( key != null )
        {
            obj = objectPool.computeIfAbsent(key, new java.util.function.Function<COSObjectKey, COSObject>() {
                public COSObject apply(COSObjectKey k) {
                    return new COSObject(k, parser);
                }
            });
        }
        return obj;
